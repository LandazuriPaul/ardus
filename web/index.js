...
class ardus {
    ...
    newWordFromCode(code) {
        let index = indexFromCode(N,code);
        let key = keyFromCode(N,code);
...
function customModulo(n,mod) {
    let result = n;
    if (mod < 0) {
        alert("Argument mod is negative.");
        return 0;
    }
    if (n < mod && n >= 0) {
        return n;
...
function encodeIndex(n,index,step,key) {
    if(index < 0 || index >= n) {
        alert("Index mauvais");
        return 0;
    } else {
        return customModulo(index+step*key,n);
    }
}

function decodeIndex(n,index,step,key) {
    if(index < 0 || index >= n) {
        alert("Index mauvais");
        return 0;
    } else {
...
function keyFromCode(n,code) {
    let nbDigits = n.toString().length;
    let string = code.toString();
    return parseInt(string.slice(nbDigits,string.length));
}

function indexFromCode(n,code) {
    let nbDigits = n.toString().length;
    let string = code.toString();
